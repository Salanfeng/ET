...
from executorch.examples.qualcomm.oss_scripts.qwen2_5_vl.model.static_llama import (
    LlamaModel,
)
...
def qwen_config_to_model_args(qwen_config, max_seq_len: int = 2048, max_batch_size: int = 1) -> ModelArgs:
    """
    Convert Qwen2.5-VL configuration to ModelArgs for LLaMA-style processing.
    
    Args:
        qwen_config: Qwen2_5_VLConfig from HuggingFace or static implementation
        max_seq_len: Maximum sequence length for the model
        max_batch_size: Maximum batch size for the model
        
    Returns:
        ModelArgs: Converted configuration for LLaMA-style model
    """
    # Extract basic model dimensions
    dim = qwen_config.hidden_size
    n_layers = qwen_config.num_hidden_layers
    n_heads = qwen_config.num_attention_heads
    n_kv_heads = qwen_config.num_key_value_heads
    vocab_size = qwen_config.vocab_size
    
    # Calculate hidden dimension for MLP
    hidden_dim = qwen_config.intermediate_size
    
    # Calculate head dimension
    head_dim = dim // n_heads
    
    # Get normalization epsilon
    norm_eps = qwen_config.rms_norm_eps
    
    # Get RoPE configuration
    rope_freq_base = qwen_config.rope_theta
    
    # Get token IDs
    bos_idx = getattr(qwen_config, 'bos_token_id', 151643)
    eos_idx = getattr(qwen_config, 'eos_token_id', 151645)
    
    # Check for scaled RoPE
    use_scaled_rope = False
    rope_scale_factor = 8  # default
    if hasattr(qwen_config, 'rope_scaling') and qwen_config.rope_scaling is not None:
        rope_type = qwen_config.rope_scaling.get('type', 'default')
        if rope_type in ['linear', 'dynamic']:
            use_scaled_rope = True
            rope_scale_factor = qwen_config.rope_scaling.get('factor', 8)
...
def compile(args, pte_filename, tokenizer):
    ...
    # Prepare models for HTP optimization using LLaMA-style model structure
    for llama_instance in llama_instance_list:
        # Apply SHA (Single Head Attention) optimization to attention layers
        for layer in llama_instance.layers:
            ...
            # Apply feedforward Conv2d optimization to MLP layers
            if hasattr(layer, 'mlp') and hasattr(layer.mlp, "prepare_feedfoward_conv"):
                layer.mlp.prepare_feedfoward_conv()
        
        # Apply output layer Conv2d optimization if available
        if hasattr(llama_instance, "prepare_output_conv"):
            llama_instance.prepare_output_conv()
