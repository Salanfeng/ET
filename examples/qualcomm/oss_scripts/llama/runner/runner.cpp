...
float Runner::compute_perplexity(
        const std::string& prompt,
        int32_t n_ctx,
        int32_t stride,
    ...
    if (encode_res.error() != tokenizers::Error::Ok)
        return -1.0f;
    std::vector<uint64_t> prompt_tokens = encode_res.get();
    ...
    size_t n_chunk = (n_tokens - n_ctx + stride - 1) / stride;
    // for (size_t i = 0; i < n_chunk; ++i) {
    //     size_t start = i * n_ctx;
    //     size_t end = start + n_ctx;
    //     std::vector<uint64_t> prefill_tokens(
    //             prompt_tokens.begin() + start, prompt_tokens.begin() + end);
        
    //     auto decoded_tokens = token_generator_->tokenizer_->decode_all(prefill_tokens);
    //     (*log_stream) << decoded_tokens << std::endl;
    // }
    // return 0.0f;
    for (size_t i = 0; i < 1; ++i) {
        std::cout << "Processing chunk " << i + 1 << " of " << n_chunk << std::endl;
        (*log_stream) << "Processing chunk " << i + 1 << " of " << n_chunk << std::endl;
        size_t start = i * stride;
        size_t prefill_end = n_ctx / 2 + start;
        size_t end = start + n_ctx;
        if (end > n_tokens)
            break;
        size_t first = n_ctx / 2;
        // 给 prompt_tokens加上BOS
        std::vector<uint64_t> prefill_tokens(
                prompt_tokens.begin() + start, prompt_tokens.begin() + prefill_end);
        auto prefill_res = prompt_processor_->prefill(prefill_tokens, 0);
        std::vector<uint64_t> window_tokens(
                prompt_tokens.begin() + start, prompt_tokens.begin() + end);
        ET_CHECK_MSG(
                prefill_tokens.size() == n_ctx / 2, "Prefill tokens size mismatch");
        token_generator_->generate_ppl(
                window_tokens, n_ctx / 2, ppl_calculator, log_stream);
    }
    ppl_calculator->finalize();
    return ppl_calculator->current_ppl;
}
